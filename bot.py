...
async def init_db():
    ...
    pool = await asyncpg.create_pool(DATABASE_URL, min_size=2, max_size=20, command_timeout=30)
    ...
    async with pool.acquire() as db:
        ...
        defaults = {
            ...
            "topup_methods": "💳 <b>طرق شحن الرصيد</b>\n\n🏦 <b>Syriatel Cash</b>\nلم يتم ضبط رقم التحويل بعد.\n\nأرسل المبلغ المطلوب، ثم حوّل إلى الرقم الموجود هنا، وبعدها أرسل رقم العملية للإدارة.",
            "withdraw_methods": "💰 <b>طرق سحب الرصيد</b>\n\n📱 <b>Syriatel Cash</b>\nيتم السحب إلى رقم Syriatel Cash الذي تدخله عند إنشاء الطلب.",
            "mandatory_channel_chat_id": "@Ban1D1",
            "mandatory_channel_title": "Ban1D1",
            "mandatory_channel_url": "https://t.me/Ban1D1",
        }
        ...
        # Seed the user's mandatory public channel without deleting existing channels.
        await db.execute(
            """INSERT INTO channels(chat_id,title,url,active) VALUES($1,$2,$3,TRUE)
               ON CONFLICT(chat_id) DO UPDATE SET title=$2,url=$3""",
            "@Ban1D1", "Ban1D1", "https://t.me/Ban1D1"
        )
...
async def main_menu(is_admin=False):
    rows = [
        ...
        [InlineKeyboardButton(text="🏠 ستارت / الرئيسية", callback_data="home")],
...
def admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="📢 القنوات", callback_data="a_channels"),
            InlineKeyboardButton(text="🔘 الأزرار", callback_data="a_buttons"),
        ],
        [
            InlineKeyboardButton(text="🎁 المكافآت", callback_data="a_settings"),
            InlineKeyboardButton(text="💳 طرق الدفع", callback_data="a_payments"),
        ],
...
async def daily(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"🎁 <b>الهدية اليومية</b>\n\nتمت إضافة <b>{reward:,} ل.س</b> إلى رصيدك.",
        parse_mode="HTML", reply_markup=back_home()
    )
    await callback.answer("✅ تم إضافة الهدية.")
...
async def gift_channels(callback: CallbackQuery):
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=(await channel_gate()).inline_keyboard + [[InlineKeyboardButton(text="⬅️ رجوع", callback_data="home")]])
    )
    await callback.answer()
...
async def withdraw_info(callback: CallbackQuery):
    ...
    methods = await get_setting("withdraw_methods", "💰 <b>طرق سحب الرصيد</b>")
    await callback.message.edit_text(
        methods + "\n\n" +
        f"📌 الحد الأدنى: <b>{minimum:,} ل.س</b>\n\n"
        "📝 لإنشاء طلب سحب أرسل:\n<code>/withdraw المبلغ رقم_سيريتل_كاش</code>\n\n"
        "مثال: <code>/withdraw 15000 09xxxxxxxx</code>",
        parse_mode="HTML", reply_markup=back_home()
    )
    await callback.answer()
...
async def topup_info(callback: CallbackQuery):
    if not await require_sub(callback):
        await callback.answer()
        return
    methods = await get_setting("topup_methods", "💳 <b>طرق شحن الرصيد</b>")
    await callback.message.edit_text(
        methods + "\n\n"
        "📝 بعد التحويل أرسل:\n<code>/topup المبلغ رقم_العملية</code>\n\n"
        "وسيصل الطلب إلى الإدارة للمراجعة والموافقة.",
        parse_mode="HTML", reply_markup=back_home()
    )
    await callback.answer()
...
async def admin_command(message: Message):
    if not is_admin(message):
        await message.answer("❌ ليس لديك صلاحية دخول لوحة الإدارة.")
        return
    await clear_admin_action(message.from_user.id)
    await message.answer("👑 <b>لوحة الإدارة</b>\n\nتحكم كامل بالبوت من هنا.", parse_mode="HTML", reply_markup=admin_menu())
...
async def admin_channels(callback: CallbackQuery):
    ...
    if not rows:
        ...
        for row in rows:
            ...
            text += f"{status}\n🆔 {row['id']} | {row['title']}\n🔗 {row['url']}\n💬 {row['chat_id']}\n\n"
    text += "➕ إضافة: /addchannel chat_id | الاسم | الرابط\n➖ تعطيل: /delchannel chat_id\n🔄 تفعيل: /enablechannel chat_id"
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=back_admin()); await callback.answer()
...
@dp.message(Command("settopup"))
async def settopup(message: Message):
    if not is_admin(message): return
    text = message.text[len("/settopup"):].strip()
    if not text:
        await message.answer("/settopup نص طرق الشحن"); return
    await set_setting("topup_methods", text)
    await message.answer("✅ تم تحديث طرق الشحن.")

@dp.message(Command("setwithdraw"))
async def setwithdraw(message: Message):
    if not is_admin(message): return
    text = message.text[len("/setwithdraw"):].strip()
    if not text:
        await message.answer("/setwithdraw نص طرق السحب"); return
    await set_setting("withdraw_methods", text)
    await message.answer("✅ تم تحديث طرق السحب.")
...
# =========================================================
# PAYMENT METHODS / FULL BOT CONTROL
# =========================================================
...
async def admin_payments(callback: CallbackQuery):
    if not is_admin(callback):
        await callback.answer("❌ غير مصرح.", show_alert=True); return
    top = await get_setting("topup_methods", "")
    wd = await get_setting("withdraw_methods", "")
    await callback.message.edit_text(
        "💳 <b>طرق الدفع والسحب</b>\n\n"
        f"<b>الشحن الحالي:</b>\n{escape(top)}\n\n"
        f"<b>السحب الحالي:</b>\n{escape(wd)}",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✏️ تعديل الشحن", callback_data="pay_edit:topup")],
            [InlineKeyboardButton(text="✏️ تعديل السحب", callback_data="pay_edit:withdraw")],
            [InlineKeyboardButton(text="⬅️ لوحة الإدارة", callback_data="admin")],
        ])
    )
    await callback.answer()
...
async def pay_edit(callback: CallbackQuery):
    if not is_admin(callback):
        await callback.answer("❌", show_alert=True); return
    kind = callback.data.split(":",1)[1]
    await set_admin_action(callback.from_user.id, "set_topup_methods" if kind == "topup" else "set_withdraw_methods")
    await callback.message.answer("📝 أرسل النص الكامل لطريقة الدفع الآن.\nيمكنك كتابة رقم Syriatel Cash والتعليمات كما تريد.\nأرسل /cancel للإلغاء.")
    await callback.answer()
...
async def admin_text_actions(message: Message):
    ...
    if action == "set_topup_methods":
        await set_setting("topup_methods", text)
        await clear_admin_action(message.from_user.id)
        await message.answer("✅ تم تحديث طرق الشحن.", reply_markup=admin_menu())
        return

    if action == "set_withdraw_methods":
        await set_setting("withdraw_methods", text)
        await clear_admin_action(message.from_user.id)
        await message.answer("✅ تم تحديث طرق السحب.", reply_markup=admin_menu())
        return
