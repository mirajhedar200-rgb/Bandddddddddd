...
async def init_db():
    ...
    try:
        ...
        await c.execute(
            """
            INSERT OR IGNORE INTO gift_codes(code,amount,max_uses)
            VALUES('WELCOME99',5000,0)
            """
        )
...
async def check_sub(uid):
    ...
    for ch in await channels():
        ...
        try:
            ...
            mem = await BOT.get_chat_member(
                ch["chat_id"],
                uid,
            )

            if mem.status not in (
                "member",
                "administrator",
                "creator",
            ):
...
async def terms(c: CallbackQuery):
    ...
    if not await check_sub(c.from_user.id):
        ...
        return await sub_prompt(c.message)
...
async def ich_pass(
    ...
    await m.answer(
        "✅ تم إنشاء حساب Ichancy الشكلي بنجاح.",
        reply_markup=main_kb(),
    )
...
async def ich_data(c):
    ...
    await c.message.answer(
        ...
        f"⚠️ الحساب شكلي داخل البوت فقط."
    )
...
async def dep_click(
    ...
    await c.message.answer(
        "💳 أدخل مبلغ الشحن بالليرة السورية\n"
        f"الحد الأدنى: {minimum:,}"
    )


@DP.message(S.dep_amount)
async def dep_amount(
    m,
    state,
):
...
@DP.callback_query(
    F.data.startswith("dep_method:")
)
async def dep_method(
    c,
    state,
):
    ...
    await c.message.answer(
        ...
        "أرسل رقم العملية/إيصال التحويل "
        "أو اكتب وصف العملية:"
    )
...
async def wd_method(
    ...
    await c.message.answer(
        "أرسل رقم الحساب/رقم الهاتف/"
        "بيانات الاستلام التي تريد السحب إليها:"
    )


@DP.message(S.wd_account)
async def wd_account(
    m,
    state,
):
...
async def a_balance(
    ...
    if admin(c.from_user.id):
        ...
        await c.message.answer(
            "أرسل:\n"
            "USER_ID | AMOUNT | bot/ichancy | add/sub\n\n"
            "مثال:\n"
            "<code>12345 | 5000 | bot | add</code>"
        )


@DP.message(S.admin_amount)
async def a_balance_msg(
    m,
    state,
):
...
async def add_method(
    ...
    await c.message.answer(
        "أرسل:\n"
        "الاسم | تفاصيل الطريقة\n\n"
        "مثال:\n"
        "<code>سيريتل كاش | 09xxxxxxxx</code>"
    )


@DP.message(S.admin_method)
async def add_method_msg(
    m,
    state,
):
...
async def settings(c):
    ...
    if admin(c.from_user.id):
        ...
        await c.message.answer(
            f"⚙️ الإعدادات\n\n"
            f"حد شحن البوت: "
            f"{await setting('min_deposit')}\n"
            f"حد سحب البوت: "
            f"{await setting('min_withdraw')}\n"
            f"حد شحن Ichancy: "
            f"{await setting('min_i_deposit')}\n"
            f"حد سحب Ichancy: "
            f"{await setting('min_i_withdraw')}\n\n"
...
async def tx_open(c):
    ...
    if admin(c.from_user.id):
        ...
        await c.message.answer(
            "استخدم 📋 طلبات الشحن أو "
            "السحب من لوحة الإدارة لمعالجة الطلبات."
        )
...
async def main():
    ...
    # إزالة أي Webhook قديم قبل بدء polling
